...
const ItemModal = ({ show, setShow, itemId, setEditItem, collectionId }) => {
  ...
  useEffect(() => {
    if (itemId) {
      getItem(itemId).then((data) => {
        ...
        setStringFieldsValues([data.item.stringField1 ?? '', data.item.stringField2 ?? '', data.item.stringField3 ?? '']);
        ...
        setDateFieldsValues([data.item.dateField1 ? data.item.dateField1.slice(0,10) : '', data.item.dateField2 ?  data.item.dateField2.slice(0,10) : '', data.item.dateField3 ? data.item.dateField3.slice(0,10) : '']);
      })
    }
  }, [itemId]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const intCustomFieldsLength = Object.values(collection).slice(5,8).filter((field) => field).length;
    const stringCustomFieldsLength = Object.values(collection).slice(8,11).filter((field) => field).length;
    const textCustomFieldsLength = Object.values(collection).slice(11,14).filter((field) => field).length;
    const dateCustomFieldsLength = Object.values(collection).slice(17,20).filter((field) => field).length;
    ...
    if (tags.length !== 0 && intCustomFieldsLength === intCustomFieldsValuesLength && stringCustomFieldsLength === stringCustomFieldsValuesLength && textCustomFieldsLength === textCustomFieldsValuesLength && dateCustomFieldsLength === dateCustomFieldsValuesLength) {
        ...
        editItem(
          {
            ...
            customFields: JSON.stringify([...intFieldsValues, ...stringFieldsValues, ...textFieldsValues, ...boolFieldsValues, ...dateFieldsValues]),
          },
          itemId
        )
